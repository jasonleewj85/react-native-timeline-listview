...
export default class Timeline extends Component {
  ...
  _renderDetail(rowData, sectionID, rowID) {
    console.log(rowData.descriptionObj);
    let title = rowData.description ? (
      <View>
        <Text style={[styles.title, this.props.titleStyle, { fontWeight: rowData.isBold ? 'bold' : 'normal', color: rowData.titleTextColor }]}>
          {rowData.title}
        </Text>
        <View style={[styles.description, this.props.descriptionStyle]}>
          ...
          {typeof rowData.description === 'function' && rowData.description()}
        </View>
      </View>
    ) : (
      <Text style={[styles.title, this.props.titleStyle, { fontWeight: rowData.isBold ? 'bold' : 'normal', color: rowData.titleTextColor }]}>{rowData.title}</Text>
    );
  ...
  _renderCircle(rowData, sectionID, rowID) {
    ...
    var iconSize = rowData.iconSize
      ? rowData.iconSize
      : this.props.iconSize ? this.props.iconSize : defaultIconSize;
    ...
    switch (this.props.columnFormat) {
      case "single-column-left":
        circleStyle = {
          ...
          left: this.state.x - circleSize / 2.1 + (lineWidth - 1) / 2
        };
        break;
      case "single-column-right":
        circleStyle = {
    ...
    return (
      <Animatable.View animation="pulse" iterationCount="infinite" duration={1500} style={[styles.circle, circleStyle, this.props.circleStyle]}>
        {innerCircle}
      </Animatable.View>
    );
  }
...
const styles = StyleSheet.create({
  ...
  detail: { minHeight: 50, paddingBottom: 10 },
  description: {
    marginTop: 5,
    marginBottom: 20
  },
